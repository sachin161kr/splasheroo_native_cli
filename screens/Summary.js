...
const Summary = ({ route }) => {
    ...
    const addPromoCode = () => {
        ...
        axios
            .request(options)
            .then((response) => {
                let expiry = moment(response.data.promoCode[0].expireDate).format('DD/MM/YYYY');
            ...
            <View className="px-4 bg-white h-full">
                ...
                <View className="flex-row mt-5 bg-[#F6FBFF] p-5 justify-between border-sm">
                    ...
                    <View>
                        ...
                        <Text className="mt-4">{moment(bookingDetails.date).format("DD-MM-YYYY")}</Text>
                        <Text className="mt-4">{bookingDetails.startTime}{"-"}{bookingDetails.endTime}</Text>
                        <Text className="mt-4">{bookingDetails.location.substring(0, 20)}</Text>
                        <Text className="mt-4 uppercase">{bookingDetails.postCode}</Text>
                ...
                <View className="mt-3 flex-row justify-between items-center">
                    ...
                    <View>
                        <TouchableOpacity className="relative bottom-0 bg-[#00BCD4] rounded-full py-4 px-5 h-100 w-100" mode="contained" onPress={addPromoCode}>
                ...
                <View className="flex-row mt-5 p-10 justify-between bg-[#F6FBFF]">
                    <View>
                        ...
                        {checkPromoCode && <Text className="text-[#00BCD4] text-lg mt-4">Promo</Text>}
                        <Text className="color-[#707070] text-lg mt-4">Total</Text>
                    </View>
                    <View>
                        <Text className="text-lg">£{data.price}</Text>
                        {checkPromoCode && <Text className="text-lg text-[#00BCD4] mt-4">-£{promoCodePrice}</Text>}
                        <Text className="text-lg mt-4">£{checkPromoCode ? discountPrice : data.price}</Text>
                    </View>
                </View>
                <View className="relative top-20 px-4 w-full text-white mt-5">
                    {isLoading ?

...
const StepForm = () => {
    ...
    const { fullName, email, password, postCode, user_address, phoneNumber } = formData;
    const { emailError, passwordError, phoneError } = error;
    const formTitles = ["What’s your name?", "What’s your email address?", "Create a password", "What’s your address?", "What’s your phone number?", "One last step, you’re almost done!"];
    ...
    const screenDisplay = () => {
        if (selectScreen === 0) {
            ...
            return <Email formData={formData} setFormData={setFormData} error={error} setError={setError} />
        } else if (selectScreen === 2) {
            return <Password formData={formData} setFormData={setFormData} error={error} setError={setError} />
        } else if (selectScreen === 3) {
            return <PostCode formData={formData} setFormData={setFormData} />
        } else if (selectScreen === 4) {
            return <PhoneNumber formData={formData} setFormData={setFormData} error={error} setError={setError} />
    ...
    return (
        ...
        <SafeAreaView className="px-4 mt-5 h-full w-full relative">
            ...
            <View>
                <Text className="text-center mt-2 color-[#5E5E5E]">{formTitles[selectScreen]}</Text>
            </View>
            ...
            <View className="absolute w-full bottom-24 px-4 text-white">
                {selectScreen < 5 ? <Button className="bg-[#00BCD4]" disabled={buttonDisabled} mode="contained" onPress={handleNext}>
                    Next
                </Button> : <Button className="bg-[#00BCD4]" disabled={!(termsChecked && drivingCheck)} mode="contained" onPress={handleSubmit}>
                    Submit
                </Button>}
            </View>
